...
isCommand h command message = case hTextFromMessage h message of
  Nothing -> False
  Just text -> isPrefixOf [command] . T.words . T.strip $ text
...
handleSettingRepetitionCount h count = do
  ...
  return [MessageResponse $ hMessageFromText h "Ok"]
...
handleRepeatCommand h = do
  ...
  st <- hGetState h
  let repeatCount = stRepetitionCount st
      defaultTitle = confRepeatReply . hConfig $ h
      title = T.replace "{count}" (T.pack . show $ repeatCount) defaultTitle
      events = liftM2 zip id (map SetRepetitionCountEvent) [1 .. 5]
  return [MenuResponse title events]
...
respondWithEchoedMessage h message = do
  ...
  st <- hGetState h
  let repeatCount = stRepetitionCount st
  return $ replicate repeatCount (MessageResponse message)